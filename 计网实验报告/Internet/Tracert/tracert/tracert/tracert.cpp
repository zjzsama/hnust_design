...
int tracert() {
	...
	string IpAddress;
	cout << "查询子网状态" << endl;
	string subnet = "192.168.1.";   //127.0.0.      192.168.139
	int startIP = 1;
	int endIP = 254;
	for (int i = startIP; i <= endIP; i++)
	{
		IpAddress = subnet + to_string(i);
		//得到 IP 地址
		u_long ulDestIP;
		// 转换不成功时按域名解析
		if (inet_pton(AF_INET, IpAddress.c_str(), &ulDestIP) <= 0) {
			...
			if (getaddrinfo(IpAddress.c_str(), NULL, &hints, &res) == 0) {
				ulDestIP = ((struct sockaddr_in*)res->ai_addr)->sin_addr.s_addr;
				freeaddrinfo(res);
			}
			else {
			...
			while (1)
			{
				...
				if (iReadDataLen != SOCKET_ERROR)//有数据到达
				{
					...
					if (DecodeIcmpResponse(IcmpRecvBuf, iReadDataLen, DecodeResult, ICMP_ECHO_REPLY, ICMP_TIMEOUT))
					{
						...
						inet_ntop(AF_INET, &DecodeResult.dwIPaddr, ipStr, sizeof(ipStr));
						cout << '\t' << ipStr << '\t' << "在线" << endl;
						ipString.push_back(ipStr);
						break;
					}
				}
				else if (WSAGetLastError() == WSAETIMEDOUT) //接收超时，输出*号
				{
					cout << " *" << '\t' << IpAddress << '\t' << "离线" << endl;
					break;
				}
				else
				{
...
int main()
{
	tracert();
	cout << endl;
	for (const string& value : ipString) {
		cout << "\tIP地址" << "\t信息" << endl;
		cout << value << "\t在线" << endl;
	}
}
